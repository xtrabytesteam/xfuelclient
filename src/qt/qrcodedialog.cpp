...
{
    QString ret = QString("XFuel:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non XFUEL unit input in lnReqAmount, we generate the URI with XFUEL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(XFuelUnits::format(XFuelUnits::XFUEL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
